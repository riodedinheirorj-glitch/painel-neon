...
import aiCorrectingImage from "@/assets/ai-correcting-addresses.png";
...
const ProcessingStep = ({ progress, status, isComplete }: ProcessingStepProps) => {
  const isAiCorrecting = status.toLowerCase().includes("corrigindo endereços com ia");
          ...
          <>
            {isAiCorrecting ? (
              <div className="w-full max-w-md space-y-4">
                <img 
                  src={aiCorrectingImage} 
                  alt="IA corrigindo endereços" 
                  className="w-full h-auto rounded-lg shadow-lg shadow-primary/30 animate-glow-pulse"
                />
                <div className="space-y-2">
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground">
                    Corrigindo e validando endereços automaticamente...
                  </p>
                </div>
              </div>
            ) : (
